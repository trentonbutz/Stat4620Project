...
Before we are able to perform Exploratory Data Analysis and investigate the Ames Housing Data, we must first load in the training and testing data as well as any packages that will be required in our analysis. Please note that all code after the first two chunks has been hidden in the report, but can be viewed in the RMD file attached to the project submission.
...
```{r}
par(mfrow=c(2,2))
boxplot(SalePrice ~ Condition1, data=train, main = 'Proximity to various conditons')
boxplot(SalePrice ~ BldgType, data=train, main = 'Building Type')
boxplot(SalePrice ~ Exterior1st , data=train, main = 'Exterior covering on house')
boxplot(SalePrice ~ KitchenQual, data=train, main = 'Kitchen Quality')
```

Taking a look at more of the categorical predictors shows that there are definite relationships between them and the dependent variable, SalePrice. In the top two box plots, it is clear that both categorical predictors have a single category with a large majority of the more highly priced houses. Top left represents the houses proximity to various locations such as parks or railroads, while top right categorizes houses based on the type of building it is. Based on intuition and these plots, it is clear that these predictors show high potential for predictive power. Looking at the bottom two plots shows lots of variation between categories of the respective predictors. This is another sign that these predictors could be influential for our final model.


```{r}
#getting only numeric vars
numeric_cols <- unlist(lapply(train, is.numeric))
numer_data <- train[,numeric_cols]
#only looking at strong/moderate correlation with dependent variable, SalePrice
cm <- cor(numer_data)[38,]
strong <- abs(cm) > 0.5 & abs(cm) < 1
cm[strong]
```

Once we've gotten a feel for the dataset and cleaned it up, its important to make sure we understand the variables. Above shows numerical variables with high correlation (above 0.6) with our dependent variable of Sale Price. The larger the magnitude of a variable's correlation, the more predictive power it can add to the model. These are the predictors that we should take some time to understand because most likely they will be used in the final model. Below is a brief summary of what these predictors represent.

* OverallQual: Overall Quality quantifies the houses overall material and finish of the house from 1-10  
* TotalBsmtSF, 1stFlrSF & GrLivArea: Square footage of the entire basement, first floor, and all living area above ground, respectively 
* YearBuilt & YearRemodAdd: The year that the house was built or remodeled, respectively
...
* GarageCars & GarageArea: Shows the number of cars and Square Footage of the garage, respectively  
    
  
Now while all of these variables show moderate to strong relationships with the dependent variable, they also overlap quite a bit. This means that some of these predictors represent that same thing as others. So, variables such as GarageCars and GarageArea are both contributing almost the same information to the model. This is something important to look out for when fitting models.


After looking closely at the data, it is clear that the data set contains various predictors that should prove to be useful for modeling the dependent variable. These useful predictors exist as both categorical and numerical variables so choosing a technique that is able to incorporate both would be ideal. Since our dependent variable is continuous, many techniques such as Linear Discriminant Analysis and Logistic Regression were eliminated from the start as they model categorical dependent variables. Our initial considerations for modeling techniques were as follows: Linear Regression, LASSO regression, Ridge Regression, Principal Components Regression, and Partial Least Squares. These techniques were chosen because they work with continuous dependent variables. Furthermore, all techniques except Linear Regression perform model selection in some form. One downside to PCR and PLS is that they only work with numeric predictors. In this case, that would mean eliminating roughly have of the predictors from the start. As proven earlier, we can see that categorical variables show potential so removing them entirely could worsen predictions. 
...
The first 3 components explain 80.13% of the variation in Y, and it levels off after that. 4-6 would be the most that would be worth considering. It looks like the lowest cross-validation error occurs at 6: 38,315.
...
Let's do prediction with the 6 component model.
...
```{r}
ggplot(data = test, aes(x=SalePrice)) + 
  geom_boxplot() + 
  labs(title = "Distribution of all Sale Prices in the data")
...
ggplot(data = df, aes(x = x, y = y)) + geom_point() + 
  labs(x = "Predicted Sale Price", y = "Actual Sale Price", title = "Predicted Sale Price vs. Actual Sale Price") + 
  geom_abline(slope = 1)
```
...
Given the relatively easy interpretability of the coefficients in the LASSO model, and keeping in mind that the LASSO model is able to zero out coefficients that are less predictive of `SalePrice`, it is fair to say that, of the models considered in this study, this one is the most useful when it comes to predicting `SalePrice`. However, it is important to keep in mind that when building statistical models, different models all have their advantages and disadvantages - for example, we may appreciate the Partial Least Squares model discussed above due to the fact that it is computationally less intensive, since we retained only several components out of dozens. Therefore, it is not necessarily productive for us to end our search by declaring that the LASSO model is the "winning" model. Rather, if we were to continue working with housing data, we would seek to employ all the knowledge that we've generated in this report in order to most effectively predict the Sale Price of Houses in Ames, Iowa.
