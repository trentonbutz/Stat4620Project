...
error = mean((y_test-pred.lasso)^2)
error
# 763,744,884
...
# only 60 predictors 
```
I looked at Ridge and LASSO and to me it seems like LASSO would be better and good for us to use just based on the fact that it gets it down to 60 variables out of the 241 there are if you include all the combinations of factor variables. I also ran 2 for loops to get rid of all factor variables not included in both design matrices because i assumed that specific categorical value would not be important if it doesn't even show up in a full set of data.